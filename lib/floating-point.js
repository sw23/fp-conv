...
class FloatingPoint {
    ...
    encode(value, options = {}) {
        ...
        // For formats with NaN at maxExponent: non-zero mantissa at maxExponent is NaN.
        // If we landed there, clamp to max normal (maxExponent, 0).
        if (biasedExponent === this.maxExponent && this.hasNaN && !this.hasInfinity && mantissaInt > 0) {
            return this.getMaxNormal(sign === 1);
        }
        ...
        const isNaN_ = this.hasNaN && atMaxExponent && mantissaInt !== 0;
    ...
    getMaxNormal(negative = false) {
        ...
        if (this.hasInfinity) {
            ...
            // maxExponent with non-zero mantissa is NaN, but mantissa=0 is a valid normal number.
            // Max normal is whichever is larger: (maxExp, 0) or (maxExp-1, maxMantissa).
            // (maxExp, 0) decodes as 1.0 * 2^(maxExp-bias)
            // (maxExp-1, maxMantissa) decodes as (1 + (2^m-1)/2^m) * 2^(maxExp-1-bias)
            // = (2 - 2^-m) * 2^(maxExp-1-bias) = (1 - 2^(-m-1)) * 2^(maxExp-bias)
            // Since (1 - 2^(-m-1)) > 1 only when m<0 (impossible), (maxExp, 0) ≥ (maxExp-1, maxMantissa)
            // when 1 ≥ (1 - 2^(-m-1)), which is always true.
            // So (maxExp, 0) is always the max normal for formats without infinity.
            mant = 0;
        }
