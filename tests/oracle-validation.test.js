...
// OCP conformance sections. These are generated by the same Fraction-based
// reference as the exact-decimal sections above, so they carry the same
// authority: the expected value is the single correctly rounded result, with no
// float detour anywhere in the pipeline.
describe('Oracle Validation - OCP conformance', () => {
  let oracleVectors;
  ...
  test('overflow behavior matches the oracle at every rounding mode', () => {
    const failures = [];
    oracleVectors.overflow_mode_encode.forEach((vector) => {
      const format = buildFormat(vector.format);
      const result = format.encodeString(vector.input, {
      ...
      const { sign, exponent, mantissa } = vector.expected;
      if (result.sign !== sign || result.exponent !== exponent || result.mantissa !== mantissa) {
        failures.push(`${vector.format} ${vector.roundingMode}/${vector.overflowMode} ` +
          `"${vector.input}": expected ${sign}/${exponent}/${mantissa}, ` +
          `got ${result.sign}/${result.exponent}/${result.mantissa}`);
      }
    });
    expect(failures).toEqual([]);
  ...
  test('E8M0 decodes every bit pattern as the oracle says', () => {
    ...
    const failures = [];
    oracleVectors.e8m0_decode.forEach((vector) => {
      const value = format.decode(0, vector.exponent, 0);
      const ok = vector.expected === 'NaN' ? Number.isNaN(value) : value === vector.expected;
      if (!ok) failures.push(`field ${vector.exponent}: expected ${vector.expected}, got ${value}`);
    });
    expect(failures).toEqual([]);
    expect(oracleVectors.e8m0_decode).toHaveLength(256);
  });

  test('E8M0 encodes as the oracle says', () => {
    const format = buildFormat('e8m0');
    const failures = [];
    oracleVectors.e8m0_encode.forEach((vector) => {
      const result = format.encodeString(vector.input, { roundingMode: vector.roundingMode });
      const { sign, exponent, mantissa } = vector.expected;
      if (result.sign !== sign || result.exponent !== exponent || result.mantissa !== mantissa) {
        failures.push(`${vector.roundingMode} "${vector.input}": ` +
          `expected ${sign}/${exponent}/${mantissa}, ` +
          `got ${result.sign}/${result.exponent}/${result.mantissa}`);
      }
    });
    expect(failures).toEqual([]);
    expect(oracleVectors.e8m0_encode.length).toBeGreaterThan(0);
  });

  test('MXINT8 decodes every bit pattern as the oracle says', () => {
    const format = buildFormat('mxint8');
    const failures = [];
    oracleVectors.mxint8_decode.forEach((vector) => {
      const value = format.decodeBits(vector.raw);
      if (value !== vector.expected) {
        failures.push(`raw ${vector.raw}: expected ${vector.expected}, got ${value}`);
      }
    });
    expect(failures).toEqual([]);
    expect(oracleVectors.mxint8_decode).toHaveLength(256);
  });

  test('MXINT8 encodes the exact decimal as the oracle says', () => {
    const format = buildFormat('mxint8');
    const failures = [];
    oracleVectors.mxint8_string_encode.forEach((vector) => {
      const result = format.encodeString(vector.input, { roundingMode: vector.roundingMode });
      if (result.intValue !== vector.expected) {
        failures.push(`${vector.roundingMode} "${vector.input}": ` +
          `expected ${vector.expected}, got ${result.intValue}`);
